...
userSchema.statics.login = async function (email, password) {
  ...
  if (!user.active)
    throw new CustomError(
      "Your account has been temporarily blocked. Please reach out to our Technical Support team for further assistance.",
      403
    );

  const PASSWORD_EXPIRY_DAYS = 90;

  const changedAt = user.password.changedAt ?? user.createdAt ?? Date.now();
  const daysSinceChange =
    (Date.now() - new Date(changedAt)) / (1000 * 60 * 60 * 24);
  ...
  if (!match) {
    ...
    if (
      ...
      throw new CustomError(
        "You've tried to login too many times with an incorrect account password, this account has been temporarily blocked for security reasons. Please reach out to our Technical Support team for further assistance.",
        429
      );
    }
