...
exports.update = async (req, res, next) => {
  try {
    ...
    if (password) {
      ...
      const history = Array.isArray(checkUser.password.history)
        ? checkUser.password.history
        : [];

      const isSameAsRecent = await Promise.all(
        history.map((oldHash) => bcrypt.compare(password, oldHash))
      );
      ...
      if (isSameAsCurrent || isSameAsRecent.includes(true)) {
        throw new CustomError("You cannot reuse your last 2 passwords.", 400);
      }

      const hashed = await bcrypt.hash(password, 10);

      const newHistory = [checkUser.password.hashed, ...history].slice(0, 2);
    ...
    if (!updatedUser)
      throw new CustomError(
        "User not found, something went wrong, during update",
        404
      );
