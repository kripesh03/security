...
exports.verifyEmail = async (req, res, next) => {
  try {
    ...
    if (!emailExist.active) throw new CustomError('Your account has been temporarily blocked.', 403, { emailVerified: isEmailVerified })

    const now = new Date()
    const day = 10 * 60 * 1000 

    if (emailExist.otp.requests >= 5 && emailExist.otp.requestDate && (now - emailExist.otp.requestDate) < day) {
      throw new CustomError('Too many OTP requests. Try again after 10 mins.', 429, { emailVerified: isEmailVerified })
    }
...
exports.verifyOTP = async (req, res, next) => {
  try {
    ...
    if (!verificationStatus[email] || !verificationStatus[email].otpToken) {
      throw new CustomError('OTP session expired. Please request a new OTP.', 403)
    }
    ...
    const emailExist = await User.findOne({ email }).exec()
    if (!emailExist || !emailExist.active) throw new CustomError('Account blocked.', 403, { otpVerified: isOtpVerified })
    ...
    if (!decoded || decoded.otp !== otp) {
      ...
      if (emailExist.otp.errorCount >= 3 && emailExist.otp.errorDate && (now - emailExist.otp.errorDate) < day) {
        ...
        throw new CustomError('Too many failed attempts. Account temporarily blocked.', 429, { otpVerified: isOtpVerified })
      }
...
exports.restPassword = async (req, res, next) => {
  try {
    ...
    if (!emailExist || !emailExist.active) throw new CustomError('Account blocked.', 403, { passwordUpdated: isPasswordUpdated })
